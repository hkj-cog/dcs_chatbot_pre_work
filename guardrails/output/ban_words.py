# Output guardrail: 3-tier fuzzy ban word check applied to LLM responses
from typing import List, Optional

from guardrails.base import OutputGuardRailBase, GuardRailResult
from guardrails.constants import _BAN_WORDS_BLOCK_MSG, _SOFT_BAN_BLOCK_MSG
...
class BanWordsGuardRail(OutputGuardRailBase):
    """Three-tier fuzzy ban-word check on LLM output. Same tier semantics as BanWordsInputGuardRail."""
    ...
    # Runs the 3-tier ban-word check on LLM output, merging per-language lists from session state
    async def process(
        ...
        if not effective_hard and not effective_soft and not effective_warn:
            log_guardrail_event(GuardRailEvent(
                guardrail_name="BanWordsGuardRail",
        ...
        if tier == "hard":
            log_guardrail_event(GuardRailEvent(
                ...
                reason=f"Hard-banned words detected in output: {', '.join(matched)}",
                snippet=text[:80],
            ))
            return GuardRailResult(is_blocked=True, blocked_reason=_BAN_WORDS_BLOCK_MSG)
        ...
        if tier == "soft":
            log_guardrail_event(GuardRailEvent(
                ...
                layer="output", action="block",
                session_id=session_id, triggered=True,
                reason=f"Soft-banned words detected in output: {', '.join(matched)}",
                snippet=text[:80],
            ))
        ...
        if tier == "warn":
            log_guardrail_event(GuardRailEvent(
                ...
                layer="output", action="allow",
                session_id=session_id, triggered=True,
                reason=f"Warn-tier words detected in output (allowed): {', '.join(matched)}",
            ))
            return GuardRailResult(is_blocked=False)
